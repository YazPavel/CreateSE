...
bool Converter::Generate (string gen, int ncell)
{
	...
	*/
	...
	if (gen == "random") {
		...
		gen_file(xcenter, ycenter, zcenter, ncell);
	} else {
...
void Converter::gen_file(vector<double>& centerx, vector<double>& centery, vector<double>& centerz, int ncell)
{
	ifstream inFile;
	inFile.open("coordinates.txt");
	if (!inFile) {
		cerr << "Unable to open file coordinates.txt";
